...
1. So if there are codes that repeats like `HOT code` it will be passed to the compiler in the v8 engine called `Turbo fan compiler` and then it will optimize the hot code in to optimized machine code then it will be executed. In old days there were a compiler called `crank shaft` and it was deprecated.
...
- Design pattern are proven solutions to common software design problems. They are structured approaches to solving challenges in coding, organization, architecture, and maintainability — especially for the larger applications.
...
## `Module`
==============
In Node.js, each module is wrapped in a function when executed.
...
3. Wrap inside IIFE
   This provides module-level scope and access to special variables like exports, module, and \_\_dirname.
4. Code Evaluation
...
| ---------------------------------------------------------- | --------------------------------------------------------------------- |
| Must use **exact same name** as exported while importing   | Can use **any name** while importing                                  |
...
- libuv is a cross-platform open-source C library that provides support for asynchronous I/O based on event loops.
...
`How the Nodejs Event lopp and Browser Event loop differ`
...
`Process.nextTick()` -> process.nextTick() is a Node.js method that allows you to schedule a callback function to be invoked when the event loop is entering to the next phase
...
process.nextTick(() => {
  console.log(
    "This runs after the current event loop cycle, but before any I/O tasks"
  );
});
...
## `Server creation`
=======================
The node core module `http` is used to create a server and the `createServer` function of http. And the createServer function have a function as an parameter and that function have two objects request and response
...
Request Header - Additonal information that send by the client to server
Request body - Data send by the client to server Only for POST, PUT, PATCH, etc
...
Cache-Control: Directs caching policies for requests and responses.
...
Response Header - Additional information send by the server to client
                  `Content-Type, Set-Cookie Cache-Control Content-Length`
...
- Express automatically setting content-type
...
text/javascript -> JavaScript (older, now mostly application/javascript)
...
When a browser sends a request to your backend, all cookies are sent in the header as a single long string
Without cookie-parser, Express cannot read or access these cookies directly.
...
- Data is available throughout the entire app lifetime
- Scoped to the Express application (not per request)

```js
const express = require("express");
const app = express();

// Set a global app value
app.locals.appName = "SlotFlow";
app.locals.version = "1.0.0";

app.get("/", (req, res) => {
  res.send(`Welcome to ${app.locals.appName} v${app.locals.version}`);
});
```

# `global`
- Available everywhere in Node.js (not just in Express)
- Use carefully — can cause side effects if overused

```js
global.config = {
  db: "mongodb://localhost:27017/slotflow",
  mode: "development",
};

app.get("/config", (req, res) => {
  res.send(`DB: ${global.config.db}, Mode: ${global.config.mode}`);
});
...
- URL-encoded form data is the format used when you submit an HTML form with
  enctype="application/x-www-form-urlencoded" (default type).
- The form data is sent in the request BODY (not the URL),
  formatted in query-string style:
  name=John%20Doe&city=New%20York `%20 is space`
- express.urlencoded() parses this string and converts it into a JS object:
  { name: "John Doe", city: "New York" }
- The resulting object is then attached to req.body
  for use in your route handlers.
  - `extended` true | false :- This option tells Express which library to use for parsing the URL-encoded data.
    | Option | Parser Used | Supports Nested Objects? | Example |
    | ----------------- | ---------------------------------- | ---------------------------| ------------------------------------------------ |
    | `extended: false` | **querystring** (built-in Node.js) | ❌ No | `user[name]=John` → `{ 'user[name]': 'John' }` |
    | `extended: true` | **qs** (external library) | ✅ Yes | `user[name]=John` → `{ user: { name: 'John' } }` |

2. Router level middleware
---

Bound to a specific router, such as userRouter, adminRouter, providerRouter like that.
...
## `Dynamic routing`
==========================
Dynamic routing refers to a route handler allowing you to handle requests for multiple resources using a single route handler path with req params.
...
`fs.access(path, callback)` -> Checks whether a file or directory exists asynchronously. Preferred over exists.
...
// Using basename without ext
console.log(path.basename(filePath));
// Output: file.txt

// Using basename with ext argument
...
`Worker Threads` In JavaScript, the main thread is single-threaded, meaning it handles all tasks, in some cases, we might want to offload heavy computations or tasks to separate threads to avoid blocking the main thread. This is where Web Workers (also referred to as Worker Threads) come into picture.
...
`Child Process`
- In Node.js, the `child_process` core module allows you to create and control child processes
- from your main Node.js process. Child processes can run:
...
- Methods provided by child_process:
...
Examples :-
GET DELETE PUT PATCH HEAD

HEAD - The HEAD method is an HTTP request method that works exactly like GET, but without returning the response body.
It asks the server for the headers only, not the actual content.

POST is not idempotent — becuase it creates new resource each time.
...
Cacheable
----------
Responses from the server should say whether they can be stored (cached) by the client. This helps avoid repeating the same request and improves speed.

Uniform interface
------------------
All APIs should look and behave in a consistent way. Same type of request → same type of response.

Layered system
-----------------
The API system is built in layers (client → load balancer → server → database). Each layer only knows the layer directly next to it.

| **Code** | **Name**              | **Category**  | **Meaning**                                          |
| -------- | --------------------- | ------------- | ---------------------------------------------------- |
| **100**  | Continue              | Informational | Request received, continue process.                  |
| **101**  | Switching Protocols   | Informational | Switching to a new protocol as requested.            |
| **200**  | OK                    | Success       | Request succeeded.                                   |
| **201**  | Created               | Success       | Resource successfully created (POST).                |
| **202**  | Accepted              | Success       | Request accepted but not completed yet.              |
| **204**  | No Content            | Success       | Successful but no response body.                     |
| **301**  | Moved Permanently     | Redirect      | Resource permanently moved to another URL.           |
| **302**  | Found                 | Redirect      | Temporary redirect.                                  |
| **304**  | Not Modified          | Redirect      | Cached resource, no need to re-fetch.                |
| **400**  | Bad Request           | Client Error  | Malformed request; invalid syntax.                   |
| **401**  | Unauthorized          | Client Error  | Authentication required / invalid token.             |
| **403**  | Forbidden             | Client Error  | You don’t have permission.                           |
| **404**  | Not Found             | Client Error  | Resource not found.                                  |
| **409**  | Conflict              | Client Error  | Conflict with resource state (e.g., duplicate data). |
| **429**  | Too Many Requests     | Client Error  | Rate-limiting triggered.                             |
| **500**  | Internal Server Error | Server Error  | Unexpected server error.                             |
| **501**  | Not Implemented       | Server Error  | Server does not support this feature.                |
| **502**  | Bad Gateway           | Server Error  | Invalid response from upstream server.               |
| **503**  | Service Unavailable   | Server Error  | Server down or overloaded.                           |
| **504**  | Gateway Timeout       | Server Error  | Upstream server didn’t respond in time.              |
